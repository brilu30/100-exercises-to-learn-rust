...
impl Ticket {
    fn is_title_empty(title: String){
        if title.is_empty() {
            panic!("Title cannot be empty");
        }      
    }
    fn is_title_long(title: String){
        if title.len() > 50 {
            panic!("Title cannot be longer than 50 bytes");
        }
    }
    fn is_description_empty(description: String){
        if description.is_empty() {
            panic!("Description cannot be empty");
        }
    }
    fn is_description_long(description: String){
    ...
    pub fn new(title: String, description: String, status: String) -> Ticket {
        Self::is_title_empty(title.clone());
        Self::is_title_long(title.clone());
        Self::is_description_empty(description.clone());
        Self::is_description_long(description.clone());
        Self::is_status_valid(status.clone());
        Ticket {
            title,
            description,
            status,
